...
class MotorController {
    ...
    public:
        MotorController(int dirPin, int stepPin, int sleepPin, int resetPin, int ms1, int ms2, int ms3, int interuptPeriodMillis): 
          dirPin(dirPin), stepPin(stepPin), sleepPin(sleepPin), resetPin(resetPin), ms1(ms1), ms2(ms2), ms3(ms3), interuptPeriodMillis(interuptPeriodMillis) {};
    ...
    private:
        ...
        const static int STEP_PULSE_TIME_MICRO = 100;
        const static int DEFAULT_MOTOR_TICK_PERIOD_MS = 750;

        int dirPin, stepPin, sleepPin, resetPin, ms1, ms2, ms3;
